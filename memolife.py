...
# Conectar ao banco de dados PostgreSQL
    ...
    with connection.cursor() as cur:
        cur.execute(
            f"SELECT * FROM Users WHERE email = 'admin' AND password = 'admin';"
        )
        if not cur.fetchall():
            cur.execute(
                f"INSERT INTO Users (email, password, first, last, username) VALUES ('admin', 'admin', 'admin', 'admin', 'admin');"
            )
            connection.commit()
        ...
        for row in tables:
            if class_name == 'Psychologists':
                ...
                instance = Consultations(id=row[0],date=row[1],id_user=row[2],id_psychologist=row[3],conn=CONN)
...
def posts_from_user_with_more_than_10_friends():
    with open('password.txt', 'r') as file:
        PASSWORD = file.read().splitlines()[0]
# Conectar ao banco de dados PostgreSQL
    CONN = psycopg2.connect(
    ...
    try:
        with CONN.cursor() as cur:
                cur.execute(
                    f"""SELECT Posts.* FROM Posts JOIN Publicated ON Posts.id = Publicated.id_post  
                    JOIN users ON users.id=Publicated.id 
                    WHERE
                    (Select count(*) FROM Friends 
                    WHERE Friends.id_user=users.id) >= 10;
                    """
                )
                posts = cur.fetchall()
                if posts != []:
                    ...
                    for post in posts:
                        ...
                        return_posts.append(p)
                    return (return_posts, 0)
                return None, None
    except:
        return -1, None
CLASSES = ['Psychologists', 'Users', 'Posts', 'Consultations']
ADMIN_FUNCTIONS = [get_all, delete_all, insert_one, posts_from_user_with_more_than_10_friends]
USER_FUNCTIONS = [
                    add_friend,
                    see_friends, 
                    create_post, 
...
def admin_options(exit_index):
    ...
    print("| 4 - Get posts from users with more than 10 friends")
...
def admin_menu():
    ...
    if answer_menu == 4:
        func = posts_from_user_with_more_than_10_friends()
    else:
        # Show all classes
        while not answer:
