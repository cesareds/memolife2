    ...
    def check_posts(self):
        try:
            with self.CONN.cursor() as cur:
                ...
                #SELECT Posts.* FROM Posts JOIN Publicated ON Posts.id = Publicated.id_post  JOIN users ON users.id=Publicated.id WHERE
                # (Select count(*) FROM Friends WHERE Friends.id_user=users.id) >= 10;
                posts = cur.fetchall()
                cur.execute(
                    f"""
                    SELECT COUNT(*) 
